"""Filesystem discovery of datasets, run directories and parametrized-tomogram runs.

Backs the web UI pickers by walking dataset roots and run roots, reporting which
directories look like datasets, which runs carry checkpoints or inference
output, and which parametrized-tomogram runs are complete.
"""
...
class DatasetBrowser:
    ...
    def param_trials(self, raw_base: str) -> dict:
        """Lists completed parametrized-tomogram runs found anywhere under a root.

        A run counts as complete when its directory holds both the extraction
